...
public class AgregarInventarioActivity extends AppCompatActivity {
    ...
    private void buscarProductoPorCodigo(String code) {
        ...
        refProd.orderByChild("codigo").equalTo(code)
    ...
    private void agregarProducto() {
        ...
        if (!nombre.isEmpty() && !codigo.isEmpty()) {
            DatabaseReference ref = FirebaseDatabase.getInstance().getReference("Productos").push();
            ...
            ref.setValue(item).addOnSuccessListener(unused -> {
                Toast.makeText(this, "Producto agregado al inventario global", Toast.LENGTH_SHORT).show();
                finish();
    ...
    private void actualizarProducto() {
        ...
        FirebaseDatabase.getInstance().getReference("Productos").child(idItemParaEditar).setValue(item)
                .addOnSuccessListener(unused -> {
                    Toast.makeText(this, "Inventario global actualizado (" + resultStock + " total)", Toast.LENGTH_SHORT).show();
                    finish();
                });
    }

    private void registrarMovimiento(String nombre, String codigo, String tipo, String cantidad) {
        String uid = (firebaseUser != null) ? firebaseUser.getUid() : "SISTEMA";
        String fecha = new SimpleDateFormat("dd/MM/yyyy HH:mm", Locale.getDefault()).format(new Date());
        DatabaseReference refMov = FirebaseDatabase.getInstance().getReference("MovimientosInventario").push();
