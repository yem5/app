...
public class AgregarPersonalActivity extends AppCompatActivity {
    ...
    EditText etNombresPersonal, etApellidosPersonal, etCumplePersonal, etTelefonoPersonal, etCorreoPersonal;
    ...
    String[] areas = {"SISTEMAS", "VENTAS", "SOPORTE TECNICO", "ADMINISTRACION", "FINANZAS", "LEGAL", "MARKETING", "MANTENIMIENTO"};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        etCumplePersonal.setOnClickListener(v -> mostrarDatePicker());
    }
    ...
    private void mostrarDatePicker() {
        ...
        android.app.DatePickerDialog datePickerDialog = new android.app.DatePickerDialog(this, (view, year1, monthOfYear, dayOfMonth) -> {
            // Display only Day and Month (Month is 0-indexed)
            String dateLabel = dayOfMonth + "-" + (monthOfYear + 1);
            etCumplePersonal.setText(dateLabel);
        }, year, month, day);
    ...
    private void inicializarVariables() {
        ...
        etCumplePersonal = findViewById(R.id.etCumplePersonal);
    ...
    private void llenarDatosEdicion(Personal p) {
        ...
        etCumplePersonal.setText(p.getCumpleanos());
    ...
    private void guardarPersonal() {
        ...
        String cumple = etCumplePersonal.getText().toString().trim();
        ...
        if (!nombres.isEmpty() && !apellidos.isEmpty()) {
            DatabaseReference ref = FirebaseDatabase.getInstance().getReference("Staff").push();
            String id = ref.getKey();

            Personal p = new Personal(id, uid, nombres, apellidos, telefono, correo, fotoBase64, area, cumple);

            ref.setValue(p).addOnSuccessListener(unused -> {
                Toast.makeText(this, "Personal registrado en Staff Global", Toast.LENGTH_SHORT).show();
                finish();
    ...
    private void actualizarPersonal() {
        ...
        if (!nombres.isEmpty()) {
            Personal p = new Personal(idParaEditar, uid, nombres, apellidos, telefono, correo, fotoBase64, area, cumple);

            FirebaseDatabase.getInstance().getReference("Staff").child(idParaEditar).setValue(p)
                    .addOnSuccessListener(unused -> {
                        Toast.makeText(this, "Staff Global actualizado", Toast.LENGTH_SHORT).show();
                        finish();
                    });
        }
    }
